<template>
        ...
        <div class="flex flex-row space-x-2 bg-slate-800 bg-opacity-85 backdrop-filter backdrop-blur-md border border-slate-700 rounded-full px-4 py-2 shadow-md">
          ...
          <n-popover trigger="click" placement="top" :show-arrow="false" style="background: transparent; border: none; box-shadow: none; padding: 0;" raw>
            ...
            <div class="layer-control-panel">
              ...
              <n-checkbox v-model:checked="showPatrolAreasLayer" class="dark-checkbox">巡逻区域</n-checkbox>
            </div>
          </n-popover>
  ...
  import {
    ...
    // 图层相关
    getDroneBatteryColor, createDronesLayer, createEventsLayer, createPatrolAreaLayers
  ...
  const renderDeckLayers = () => {
    ...
    if (showPatrolAreasLayer.value && tempPatrolArea.value && tempPatrolArea.value.geojson && tempPatrolArea.value.geojson.geometry && tempPatrolArea.value.geojson.geometry.coordinates) {
      layers.push(
        new PolygonLayer({
          id: 'temp-patrol-area',
          data: [{
